...
public class M64 : IM64
{
    ...
    {
        if (!IsControllerPresent(controller)) throw new Exception($"Controller '{controller}' is not present.");
    ...
    public IEnumerable<InputModel> GetControllerInputs(Controller controller)
    {
        ...
        if (ControllerInputs.Count == 0) return inputs;

        var offset = (int)controller;
        for (var i = 0; i < ControllerInputs.Count; i += (int)ControllerCount) inputs.Add(ControllerInputs[i + offset]);
